...
def fetch_listing(item_id):
    """Fetch eBay listing using Playwright headless browser for full JS rendering."""
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # Use domcontentloaded instead of networkidle — much faster, never hangs
            page.goto(url, wait_until="domcontentloaded", timeout=20000)
            # Short fixed wait for lazy images to populate
        ...
        print(f"  Playwright error for {item_id}: {e}")
        # Fallback to urllib if Playwright fails
        try:
            req = urllib.request.Request(url, headers=HEADERS)
            with urllib.request.urlopen(req, timeout=15) as r:
                return r.read().decode("utf-8", errors="replace")
        except Exception as e2:
            print(f"  Fallback also failed for {item_id}: {e2}")
            return None
...
def parse_listing(html, item_id, fallback_title, fallback_price):
    ...
    # Check if sold/ended — be very conservative, only mark sold on definitive signals
    # If eBay blocks the fetch we get a login/redirect page — must NOT mark as sold
    page_has_price   = bool(re.search(r'US [\$][\d,]+', html))
    page_has_buy_now = bool(re.search(r'Buy It Now|buyitnow|BuyItNow', html, re.I))
    page_is_active   = page_has_price or page_has_buy_now
    explicitly_ended = bool(re.search(r'This listing has ended|listing-ended|item-ended', html, re.I))
    # Only mark sold if explicitly ended AND page loaded properly (has eBay content)
    sold = explicitly_ended and bool(re.search(r'ebay\.com|eBay', html))
    # If page looks like a bot block / redirect (no price, no eBay content) treat as active
    if not page_is_active and not explicitly_ended:
        print(f"    Warning: page for {item_id} looks blocked/redirected — keeping active")
        sold = False
    ...
    all_ebay_imgs = re.findall(
        r'https://i\.ebayimg\.com/images/g/([A-Za-z0-9~_-]+)/s-l[0-9]+\.(?:jpg|webp)',
        html
    )
    # Also check thumbs path
    all_ebay_imgs += re.findall(
        r'https://i\.ebayimg\.com/thumbs/images/g/([A-Za-z0-9~_-]+)/s-l[0-9]+\.(?:jpg|webp)',
        html
    )
...
def build_html(active, sold_list):
    ...
    def active_tile(item):
        ...
        _id  = item['id']
        _alt = item['title'].replace('"', '').replace("'", '')
        _cb  = "openLightbox('" + _id + "',0)"
        if first_img:
            img_html = '<img class="tile-main-img" src="' + first_img + '" alt="' + _alt + '" loading="lazy" onclick="' + _cb + '"/>'
        else:
